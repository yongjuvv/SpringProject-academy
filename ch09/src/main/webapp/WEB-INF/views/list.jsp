	...
	<tr><td><%-- ${board.num} --%>${num}
		...
		<c:if test="${board.del == 'y' }">
			...
			<c:if test="${board.readcount > 30 }">
				...
				<a href="view.html?num=${board.num}&pageNum=${pb.currentPage}"
...
<div align="center">
	<ul class="pagination">
		<c:if test="${pb.startPage > pb.pagePerBlock}">
			<li><a href="list.html?pageNum=1&search=${board.search}&keyword=${board.keyword}">
					<span class="glyphicon glyphicon-backward"></span>
				</a></li>
			<li><a href="list.html?pageNum=${pb.startPage - 1}&search=${board.search}&keyword=${board.keyword}">
					<span class="glyphicon glyphicon-triangle-left"></span>
				</a></li>
		</c:if>
		<c:forEach var="i" begin="${pb.startPage}" end="${pb.endPage }">
			<c:if test="${i==pb.currentPage}">
				<li class="active"><a href="list.html?pageNum=${i}&search=${board.search}&keyword=${board.keyword}">${i}</a></li>
			</c:if>
			<c:if test="${i!=pb.currentPage}">
				<li><a href="list.html?pageNum=${i}&search=${board.search}&keyword=${board.keyword}">${i}</a></li>
			</c:if>		
		</c:forEach>
		<c:if test="${pb.endPage < pb.totalPage}">
			<li><a href="list.html?pageNum=${pb.endPage + 1}&search=${board.search}&keyword=${board.keyword}">
					<span class="glyphicon glyphicon-triangle-right"></span>
				</a></li>
			<li><a href="list.html?pageNum=${pb.totalPage}&search=${board.search}&keyword=${board.keyword}">
...
<a href="insertForm.html?pageNum=1" class="btn btn-info">게시글 입력</a>
</div>
</body>
</html>
