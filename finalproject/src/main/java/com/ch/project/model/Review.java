package com.ch.project.model;

import java.util.Date;

public class Review {
	private int reviewNum;
	private String email;
	private int movNum;
	private int tvNum;
	private int hashNum;
	private int hashNum1;
	private int hashNum2;
	private int hashNum3;
	private String reviewComm;
	private Date reviewDate;
	private Date reviewUpdate;
	private int reviewFirstChk;
	private String reviewSpoiler;
	private int reviewRate;
	private int reviewLike;
	private String hashName1;
	private String hashName2;
	private String hashName3;
	private String movName;
	private String movPic;
	private String tvPic;
	private String tvName;
	private String nickName;
	private Date movDate;
	private Date tvDate;
	private int likeChk;
	private int movTime;
	private String personName;
	private String movGenre1;
	private String movGenre2;

	public int getMovTime() {
		return movTime;
	}

	public void setMovTime(int movTime) {
		this.movTime = movTime;
	}

	public String getPersonName() {
		return personName;
	}

	public void setPersonName(String personName) {
		this.personName = personName;
	}

	public String getMovGenre1() {
		return movGenre1;
	}

	public void setMovGenre1(String movGenre1) {
		this.movGenre1 = movGenre1;
	}

	public String getMovGenre2() {
		return movGenre2;
	}

	public void setMovGenre2(String movGenre2) {
		this.movGenre2 = movGenre2;
	}

	public String getNickName() {
		return nickName;
	}

	public void setNickName(String nickName) {
		this.nickName = nickName;
	}

	public int getReviewNum() {
		return reviewNum;
	}

	public void setReviewNum(int reviewNum) {
		this.reviewNum = reviewNum;
	}

	public String getEmail() {
		return email;
	}

	public void setEmail(String email) {
		this.email = email;
	}

	public int getMovNum() {
		return movNum;
	}

	public void setMovNum(int movNum) {
		this.movNum = movNum;
	}

	public int getTvNum() {
		return tvNum;
	}

	public void setTvNum(int tvNum) {
		this.tvNum = tvNum;
	}

	public int getHashNum() {
		return hashNum;
	}

	public void setHashNum(int hashNum) {
		this.hashNum = hashNum;
	}

	public int getHashNum1() {
		return hashNum1;
	}

	public void setHashNum1(int hashNum1) {
		this.hashNum1 = hashNum1;
	}

	public int getHashNum2() {
		return hashNum2;
	}

	public void setHashNum2(int hashNum2) {
		this.hashNum2 = hashNum2;
	}

	public int getHashNum3() {
		return hashNum3;
	}

	public void setHashNum3(int hashNum3) {
		this.hashNum3 = hashNum3;
	}

	public String getReviewComm() {
		return reviewComm;
	}

	public void setReviewComm(String reviewComm) {
		this.reviewComm = reviewComm;
	}

	public Date getReviewDate() {
		return reviewDate;
	}

	public void setReviewDate(Date reviewDate) {
		this.reviewDate = reviewDate;
	}

	public Date getReviewUpdate() {
		return reviewUpdate;
	}

	public void setReviewUpdate(Date reviewUpdate) {
		this.reviewUpdate = reviewUpdate;
	}

	public int getReviewFirstChk() {
		return reviewFirstChk;
	}

	public void setReviewFirstChk(int reviewFirstChk) {
		this.reviewFirstChk = reviewFirstChk;
	}

	public String getReviewSpoiler() {
		return reviewSpoiler;
	}

	public void setReviewSpoiler(String reviewSpoiler) {
		this.reviewSpoiler = reviewSpoiler;
	}

	public int getReviewRate() {
		return reviewRate;
	}

	public void setReviewRate(int reviewRate) {
		this.reviewRate = reviewRate;
	}

	public int getReviewLike() {
		return reviewLike;
	}

	public void setReviewLike(int reviewLike) {
		this.reviewLike = reviewLike;
	}

	public String getHashName1() {
		return hashName1;
	}

	public void setHashName1(String hashName1) {
		this.hashName1 = hashName1;
	}

	public String getHashName2() {
		return hashName2;
	}

	public void setHashName2(String hashName2) {
		this.hashName2 = hashName2;
	}

	public String getHashName3() {
		return hashName3;
	}

	public void setHashName3(String hashName3) {
		this.hashName3 = hashName3;
	}

	public String getMovName() {
		return movName;
	}

	public void setMovName(String movName) {
		this.movName = movName;
	}

	public String getMovPic() {
		return movPic;
	}

	public void setMovPic(String movPic) {
		this.movPic = movPic;
	}

	public String getTvPic() {
		return tvPic;
	}

	public void setTvPic(String tvPic) {
		this.tvPic = tvPic;
	}

	public String getTvName() {
		return tvName;
	}

	public void setTvName(String tvName) {
		this.tvName = tvName;
	}

	public Date getMovDate() {
		return movDate;
	}

	public void setMovDate(Date movDate) {
		this.movDate = movDate;
	}

	public Date getTvDate() {
		return tvDate;
	}

	public void setTvDate(Date tvDate) {
		this.tvDate = tvDate;
	}

	public int getLikeChk() {
		return likeChk;
	}

	public void setLikeChk(int likeChk) {
		this.likeChk = likeChk;
	}

}