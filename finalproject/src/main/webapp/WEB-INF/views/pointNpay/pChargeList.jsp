									...
									<div class="UserPage_WallPaperRow">
										<div class="UserPage_WallPaperTitle">
											<h2 class="myPointListTitle">
													포인트<br> <span class="myPointListTitle_Title">충전 및 사용 내역</span>
											</h2>
										</div>
									</div>
								</div>
								<div class="UserPageGrid">
										<div class="UserPageRow">
											<div class="container" align="center">
												<h3 class="ment_hello">
													나의 현재 보유 포인트는 <span class="ment_emphasis">
														${memPoint}</span>점이에요
												</h3>
												<table class="table" style="font-size: 14px;">
													<tr>
														...
														<th width="20%">충전</th>
														<th width="20%">사용</th>
														<th width="38%">사용 내역</th>

													</tr>

													<c:if test="${empty list}">
														<tr>
															<td colspan="5" class="noPointList">포인트 충전 및 사용 내역이 없어요 ༼☯﹏☯༽</td>
														</tr>
													</c:if>

													<c:forEach var="pcharge" items="${list}">
														<tr>
															<td><fmt:formatDate value="${pcharge.pChargeDate }"
																	pattern="M월 d일 h시 m분" /></td>

															<c:if test="${pcharge.pProductPoint > 0}">
																<td>${pcharge.pProductPoint}p</td>
																<td></td>
																<td style="color: #707070; text-align: left;">${pcharge.pDesc }</td>
															</c:if>

															<c:if test="${pcharge.pProductPoint==0}">
																<td></td>
																<c:if test="${pcharge.tvEpNum == 0}">
																	<td>${pcharge.movPrice }p</td>
																	<td style="color: #707070; text-align: left;"><a
																		href="movSearchResult.do?movNum=${pcharge.pProductNum}">${pcharge.pDesc}</a></td>
																</c:if>
																<c:if test="${pcharge.tvEpNum > 0}">
																	<td>${pcharge.tvEpPrice}p</td>
																	<td style="color: #707070; text-align: left;"><a
																		href="tvSearchResult.do?tvNum=${pcharge.pProductNum}">${pcharge.pDesc}</a>${pcharge.tvEpNum}화</td>
																</c:if>
											...
											<div class="container pageList" align="center">
												<c:if test="${startPage > pagePerBlock }">
													<a href="pChargeList.do?pageNum=${startPage-1}"> <span>이전</span></a>
												</c:if>
												<c:forEach var="i" begin="${startPage}" end="${endPage}">
													<a href="pChargeList.do?pageNum=${i}"> <span>${i}</span></a>
												</c:forEach>
												<c:if test="${endPage < totPage }">
													<a href="pChargeList.do?pageNum=${endPage+1}"> <span>다음</span></a>
												</c:if>
											</div>
